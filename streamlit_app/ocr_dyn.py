...
# =========================================================
def run_ocr_extraction(session_dir, fast_mode=True, rate_limit=4):
    """
    Wrapper function to run OCR extraction as an imported module.
    ...
    """
    global SESSION_DIR, SOURCE_FOLDER, OUT_JSON
    
    # Set dynamic paths
    SESSION_DIR = Path(session_dir)
    SOURCE_FOLDER = SESSION_DIR / "uploads"
    OUT_JSON = SESSION_DIR / "gemini_output.json"
    ...
    try:
        ...
        if OUT_JSON.exists():
            print(f"✅ OCR extraction completed: {OUT_JSON}")
            return True, [OUT_JSON]
        else:
            print(f"⚠️ Output file not found: {OUT_JSON}")
            return False, []
