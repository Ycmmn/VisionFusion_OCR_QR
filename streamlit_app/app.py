...
# =========================================================
# 🔹 Run Imported Function (NEW)
# =========================================================
def run_imported_function(func, session_dir, log_area, status_text, script_display_name="", fast_mode=True, rate_limit=4):
    """Run imported function instead of subprocess"""
    ...
    try:
        ...
        success, output_files = func(session_dir, fast_mode=fast_mode, rate_limit=rate_limit)
        ...
        if success:
            ...
            # Display output info in log area
            if output_files:
                log_output = f"✅ Created {len(output_files)} file(s):\n"
                for f in output_files:
                    log_output += f"  - {Path(f).name}\n"
                log_area.code(log_output, language="bash")
            
            return True
        else:
            status_text.markdown(f"""
            <div class="status-box status-warning">⚠️ {script_display_name} encountered an issue</div>
            """, unsafe_allow_html=True)
            return False
        ...
        """, unsafe_allow_html=True)
        import traceback
        log_area.code(traceback.format_exc(), language="bash")
        return False
    ...
    if st.button("🚀 Start Processing", type="primary"):
        ...
        try:
            if pipeline_type == 'excel':
                ...
                # 🔹 Use imported function instead of subprocess
                success = run_imported_function(
                    run_excel_mode,
                    session_dir,
                    log_area,
                    status_text,
                ...
                for stage_name, func, progress_val in stages:
                    ...
                    stage_success = run_imported_function(
                        func, 
                        session_dir, 
                        log_area, 
                        status_text,
